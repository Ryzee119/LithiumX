...
static size_t g_rxCallbackBeginIndex;
...
static atomic_size_t g_rxBuffPoolHead;
...
static void *nvnetdrv_rx_pop (void)
{
    ...
    return g_rxBuffPool[g_rxBuffPoolHead--];
}
...
static void __stdcall nvnetdrv_dpc (PKDPC Dpc, PVOID DeferredContext, PVOID arg1, PVOID arg2)
{
...
static void NTAPI rxrequeue_thread(PKSTART_ROUTINE StartRoutine, PVOID StartContext)
{
    ...
    while (true) {
        //Wait until there is an empty descirptor in the RX ring
        KeWaitForSingleObject(&g_rxRingFreeDescriptors, Executive, KernelMode, FALSE, NULL);
        if (!g_running)
            break;

        //Requeue it with a buffer from the RX buffer stack.
...
static void NTAPI rxcallback_thread(PKSTART_ROUTINE StartRoutine, PVOID StartContext)
{
    ...
    while (true) {
        ...
        //Call all queued RX callbacks
        g_rxCallback(g_rxCallbackQueue[g_rxCallbackBeginIndex].bufAddr,
                        g_rxCallbackQueue[g_rxCallbackBeginIndex].length);
        g_rxCallbackBeginIndex = (g_rxCallbackBeginIndex + 1) % g_rxBuffCnt;
    }
    PsTerminateSystemThread(0);
}
...
int nvnetdrv_init(size_t rx_buffer_count, nvnetdrv_rx_callback_t rx_callback)
{
    ...
    // The NIC hardware IRQ queues a DPC. The DPC then sets g_irqEvent. g_irqEvent is monitored by irqthread to handle the IRQ
    ...
    // Setup the push/pop stack for all our RX buffers. RX buffers are stored here until they can be pushed into the RX ring.
    // We need to handle the case where the user supplies less buffers than can fit in the RX ring, also if excess (spare) buffers are supplied.
    g_rxBuffPoolHead = -1;
...
void nvnetdrv_stop(void)
{
    ...
    // Pass back all TX buffers to network stack.
...
{
    NTSTATUS status;
    LARGE_INTEGER no_sleep = {.QuadPart = 0};
    ...
    while (true) {
        ...
        //Did the user shutdown nvnet while we were waiting?
        if (!g_running)
            return false;

        // We want to try claim all tx descriptors at once wihtout sleeping.
        size_t i = 0;
        for (i = 0; i < count - 1; i++) {
        ...
        //Did the user shutdown nvnet while we claiming tx descriptors?
        if (!g_running)
            return false;
...
void nvnetdrv_submit_tx_descriptors (nvnetdrv_descriptor_t *buffers, size_t count)
{
    ...
    while (!atomic_compare_exchange_weak(&g_txRingEndIndex, &descriptors_index, (descriptors_index + count) % TX_RING_SIZE))
        ;
...
{
    if (!g_running)
        return; // Network stack might call this after we have shutdown. Just leave.
